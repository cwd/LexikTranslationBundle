 ...
 * @Route("/condolence")
 */
class CondolenceController extends CwdController
{
    ...
    {
        $form = $this->createForm('aspetos_legacy_form_condolence', $object);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                return $this->redirect($this->generateUrl('aspetos_legacy_condolence_list', array('uid' => $request->get('obituary'))));
            } catch (\Exception $e) {
    ...
    public function toogleWinnerAction(BookEntry $bookEntry, Request $request)
    {
        ...
        return new JsonResponse(array('success' => true, 'message' => ($state) ? 'Condolence now hidden' : 'Condolence now unhidden'));
    }
    ...
    {
        $dataTable = $this->get('aspetos.legacy.grid.condolence');
        $dataTable->uid = $uid;
        $dataTable->get();

        $bookId = $this->getService()->findBookForObituary($uid, 'condolence');
