 ...
 *
 * @DI\Service("aspetos.legacy.grid.obituary")
 */
class Obituary extends Grid
{
    ...
    public function get()
    {
        ...
        $datatable
            ...
            ->setWhere('x.userCategory = :type', array('type' => 'dead'))
            ->setFields(
                array(
                    'UID' => 'x.uid as xid',
                    'Country' => 'x.domain',
            ...
            ->setRenderers(
                array(
                    10 => array(
                        'view' => 'AspetosLegacyBundle:Obituary:_actions.html.twig',
                        'params' => array(
                            'view_route'     => 'aspetos_legacy_obituary_detail',
                            'edit_route'     => 'aspetos_legacy_obituary_edit',
                            'candle_route'   => 'aspetos_legacy_candle_list',
                            'condolence_route'   => 'aspetos_legacy_condolence_list',
                            //'delete_route'   => 'aspetos_legacy_obituary_delete',
                            //'undelete_route' => 'aspetos_legacy_obituary_undelete',
                        ),
                    ),
                )
            )
                ...
                function (&$data) use ($instance)
                {
                    foreach ($data as $key => $value) {
                        ...
                        if ($key == 1) {
                            ...
                            $data[$key] = '<span style="display:none">'.$value.'</span><input type="checkbox" class="make-switch" data-on-text="Ja" data-off-text="Nein" name="mark-winner" value="1" data-off-color="success" data-on-color="danger" '.$checked.' data-id="'.$data[0].'" alt="'.$value.'">';
                        }
                    }
                }
            )
